...
const DashboardShell: React.FC<DashboardShellProps> = ({ children }) => {
  ...
  const handleGenerateShortId = async () => {
    ...
    try {
      ...
      try {
        const sidBox = new Uint8Array([0x73, 0x69, 0x64, 0x5f, ...sidBytes]) // "sid_" + shortId
        const adrBox = new Uint8Array([0x61, 0x64, 0x72, 0x5f, ...algosdk.decodeAddress(activeAddress).publicKey]) // "adr_" + address
        await client.send.registerShortId({
          ...
          boxReferences: [
            ...
            { appId: BigInt(mapperAppId), name: adrBox },
          ],
        })
      ...
      enqueueSnackbar(`Transaction failed: ${msg}`, { variant: 'error' })
    } finally {
