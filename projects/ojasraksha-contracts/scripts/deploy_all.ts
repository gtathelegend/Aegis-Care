...
export async function deployAll() {
  ...
  // 1. AuditLog
  const auditLogDeployResult = await new AuditLogFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const auditAppId = auditLogDeployResult.appClient.appId;
  console.log(`✅ AuditLog: ${auditAppId}`);

  // 2. QueueManager
  const qManagerDeployResult = await new QueueManagerFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const queueAppId = qManagerDeployResult.appClient.appId;
  console.log(`✅ QueueManager: ${queueAppId}`);

  // 3. HealthcareRBAC
  const hRbacDeployResult = await new HealthcareRbacFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const rbacAppId = hRbacDeployResult.appClient.appId;
  console.log(`✅ HealthcareRBAC: ${rbacAppId}`);

  // 4. DataFiduciaryRegistry
  const fdRegDeployResult = await new DataFiduciaryRegistryFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const fidAppId = fdRegDeployResult.appClient.appId;
  console.log(`✅ DataFiduciaryRegistry: ${fidAppId}`);

  // 5. ConsentManager
  const consentDeployResult = await new ConsentManagerFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const consentAppId = consentDeployResult.appClient.appId;
  console.log(`✅ ConsentManager: ${consentAppId}`);

  // 6. MedicalRecords
  const medRecDeployResult = await new MedicalRecordsFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const medAppId = medRecDeployResult.appClient.appId;
  console.log(`✅ MedicalRecords: ${medAppId}`);

  // 7. DataAccessManager
  const dataAccDeployResult = await new DataAccessManagerFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const accessAppId = dataAccDeployResult.appClient.appId;
  console.log(`✅ DataAccessManager: ${accessAppId}`);

  // 8. WalletMapper
  const walletMapDeployResult = await new WalletMapperFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  const walletAppId = walletMapDeployResult.appClient.appId;
  console.log(`✅ WalletMapper: ${walletAppId}`);

  // 9. VolunteerRegistry
  const volRegDeployResult = await new VolunteerRegistryFactory(commonParams).deploy({ onUpdate: 'append', onSchemaBreak: 'append' });
  ...
  await dataAccDeployResult.appClient.send.bootstrap({ args: { consentManagerAppId: consentAppId, auditAppId: auditAppId, queueAppId: queueAppId } });
  console.log('✅ Bootstraps completed.');


  // Write back App IDs to frontend .env
  console.log('📝 Updating Frontend Environment Variables...');
  const frontendEnvPath = path.resolve(__dirname, '../../ojasraksha-frontend/.env');
  let envData = '';
